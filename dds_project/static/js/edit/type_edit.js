document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleEdit(row) {
        ...
        const name = row.querySelector('.type-name').textContent;

        // Заменяем содержимое ячеек на форму редактирования
        row.querySelector('.type-name').innerHTML = `<input type="text" class="form-control edit-name-input" value="${name}">`;
    ...
    function handleCancel(row) {
        ...
        row.querySelector('.type-name').textContent = name;
        row.querySelector('.button-container').innerHTML = `
            <button class="btn btn-outline-info mr-2 edit-button">Редактировать</button>
            <button class="btn btn-outline-danger ml-2 delete-button">Удалить</button>
        `;
